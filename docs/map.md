...
| `tools/voices/reference.py` | Builds the villager-voice-cloning reference WAV from vanilla's own grunt clips, read from the client's asset cache at generation time (`AUDIO-DEC-006`, `docs/spec/domains/audio.md`). | `load_asset_index(index_path)`, `find_default_asset_index(assets_root)`, `resolve_object(clip_hash, assets_root)`, `resolve_clips(clip_names, index, assets_root)`, `build_reference_wav(clip_paths, out_wav, tmp_dir)`, `build_named_reference(reference_set, out_wav, tmp_dir, assets_root, asset_index_path)` |
| `tools/voices/render.py` | The voice pipeline (`docs/spec/domains/audio.md` §3, `AUDIO-DEC-004`/`AUDIO-DEC-005`/ `AUDIO-DEC-006`), two engines: `piper` (the original, rounds 1-3) and `chatterbox` (round four's voice-cloning engine, `AUDIO-DEC-006`, the current default choice though not the CLI default flag value — see `--engine` below). | `load_catalogue_entries()`, `load_catalogue()`, `normalize_spoken_text(text)`, `derive_input_text(line_id, subtitle, spoken)`, `derive_seed(line_id)`, `render_line(engine, line_id, text, out_ogg, chain)`, `chains_for_engine(engine)`, `run_sample(out_dir, engine, chains)`, `run_batch(engine, chain)`, `main()` |
