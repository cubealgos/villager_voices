...
| `tools/voices/render.py` | The Piper voice pipeline (`docs/spec/domains/audio.md` §3, `AUDIO-DEC-004`). | `load_catalogue()`, `derive_input_text(line_id, subtitle)`, `render_line(model_name, text, out_ogg, chain)`, `run_sample(out_dir, models, chains)`, `run_batch(model, chain)`, `main()` |
