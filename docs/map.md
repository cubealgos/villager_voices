...
| `tools/voices/render.py` | The Piper voice pipeline (`docs/spec/domains/audio.md` §3, `AUDIO-DEC-002`). | `load_catalogue()`, `derive_input_text(line_id, subtitle)`, `render_line(model_name, text, out_ogg)`, `run_sample(out_dir, models)`, `run_batch(model)`, `main()` |
...
| `tools/voices/test_render.py` | Unit tests for the input-text transform and catalogue loading (`tools/voices/render.py`). |  |
