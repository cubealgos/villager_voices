...
|---|---|---|
| Which exact `open_warm`-family refinement (`open_warm_body` with/without `noisered`/gate) for the clone engine | the batch | round eight's samples (`tools/voices/VOICES.md` "Round 8"), Kevin's listen — the reference (giordano, no lowpass in preparation), the chain family (`open_warm`), and the delivery setting (exag 0.5/cfg 0.2/temp 0.8) are all decided (`AUDIO-DEC-006` final amendment) |
