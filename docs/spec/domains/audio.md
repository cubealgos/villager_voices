...
| Reference (clone engine only) | **Decided: the `all` family** — Kevin, round four: "the `all` samples sound the best." A single WAV concatenating a chosen set of vanilla villager clips with 150ms silence between each, normalized, resolved from the client's own asset cache at generation time (`~/.gradle/caches/fabric-loom/assets/`) and never bundled, committed, or copied into the repo or a deliverable (`COMP-REQ-002`, `tools/voices/reference.py`). Round four tried three sets (`all` — every clip, `talking` — idle+haggle+yes, `idle` — idle only); round five refines the winner to `all_warm` (`all` minus the four clipped `hit*` clips, which measured as this villager's loudest/harshest source material) with a gentle `lowpass 7000` baked into the reference itself before the engine ever sees it, so the clone doesn't learn the ogg encoder's high-frequency crunch. A `tempo 0.9`-slowed reference variant was also tried and measured clearly worse (a much higher "metallic" spectral ratio, `tools/voices/VOICES.md` "Round 5") — dropped. |
| Timbre target (round five, `AUDIO-DEC-006` amendment) | **Warm and soft, not harsh/robotic/metallic.** Kevin, round four: "I find them a bit too harsh and robotic and metallic; I rather want them to sound warm and soft, but this is the best round so far." Addressed at both ends of the pipeline: at generation time, lower `exaggeration` (0.3-0.4, down from round four's 0.5) with `cfg_weight` 0.5 and a raised `temperature` (round four's default 0.8 was too crisp; round five's two carried-through settings are 1.0 and 1.1) — `render.py`'s new `--temperature` flag (`DEFAULT_TEMPERATURE`); at post-processing, three new `CLONE_POST_CHAINS` candidates aimed at less energy in the 2.5-5kHz "metallic" band relative to 150-600kHz body, without starving 1-2kHz (intelligibility): `warm` (highpass, lowpass 5500, a low-body EQ lift, a metallic-band EQ cut, a treble rolloff, a gentle compander, normalize), `soft` (`warm` but a darker lowpass 4500 and a touch of room reverb), `plain-warm` (only the two EQ moves, the minimal version). Measured ratios and Kevin's pick are in `tools/voices/VOICES.md` "Round 5" / `voices-samples-5/README.md` (scratchpad). |
...
|---|---|---|
| Which generation settings (exaggeration/cfg_weight/temperature) and post-processing chain (`warm`/`soft`/`plain-warm`) for the clone engine | the batch | round five's samples (`tools/voices/VOICES.md` "Round 5"), Kevin's listen — the reference set (`all_warm`) and the timbre target (warm and soft) are decided (`AUDIO-DEC-006` amendment) |
...
- `AUDIO-DEC-006` — **Clone the villager's own voice: a permissively licensed zero-shot cloning TTS,
  ...
  **Round five amendment** (Kevin, 2026-09-20, on round four's samples): "the `all` samples sound
  ...
  candidates). Still open: which of the two carried-through settings and which chain Kevin approves
  (§7).
