...
| Post-processing chain, final (round seven, `AUDIO-DEC-006` final amendment) | **`villager_mild`/`villager_pitch` retired — the wide-band `open` family replaces them.** Every chain through round six had a lowpass well under 10kHz; round seven's `open`/`open_warm`/`open_tempo` chains explicitly avoid any lowpass under 10kHz and drop round six's 1200Hz nasal boost entirely, the opposite move from every prior round's design (`tools/voices/render.py` `CLONE_POST_CHAINS`). `dry` (an explicit high-quality upsample from Chatterbox's native 24kHz plus `norm -3`, nothing else) is the control. Ratios, Kevin's pick, and the measurement method are in `tools/voices/VOICES.md` "Round 7" / `voices-samples-7/README.md` (scratchpad). |
...
|---|---|---|
| Which post-processing chain (`open`/`open_warm`/`dry`/`open_tempo`) and which generation settings for the clone engine | the batch | round seven's samples (`tools/voices/VOICES.md` "Round 7"), Kevin's listen — the reference is fully decided (giordano, no lowpass in preparation) and round six's `villager_mild`/`villager_pitch`/`soft` chains are superseded by round seven's "tin can" finding (`AUDIO-DEC-006` final amendment) |
